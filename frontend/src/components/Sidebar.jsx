...
const navSections = [
  ...
  },
  {
    label: "Workspace",
    items: [
      { to: "/dashboard/jobs",         label: "Jobs",         icon: "jobs",         badge: "Soon" },
      { to: "/dashboard/applications", label: "Applications", icon: "applications", badge: "Soon" },
    ],
  },
  {
    label: "Account",
    items: [
      ...
      { to: "/dashboard/settings",     label: "Settings",     icon: "settings",     badge: "Soon" },
    ],
  },
];
        ...
        <>
          ...
          {!collapsed && (
            <>
              ...
              {badge && (
                <span className="text-[10px] font-semibold font-mono text-amber-400/60 border border-amber-400/20 px-1.5 py-0.5 rounded-full leading-none">
                  {badge}
                </span>
              )}
            </>
          ...
          {collapsed && (
            <span className="pointer-events-none absolute left-full ml-3 px-2.5 py-1.5 bg-slate-800 border border-slate-700 text-slate-200 text-xs rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-150 z-50 shadow-xl">
              {label}
              {badge && <span className="ml-1.5 text-amber-400/70">{badge}</span>}
            </span>
          )}
        </>
      )}
          ...
          <div key={section.label}>
            ...
            <ul className="space-y-0.5">
              {section.items.map((item) => (
                <li key={item.to}>
                  <SidebarLink {...item} collapsed={collapsed} onClick={closeMobile} />
                </li>
              ))}
            </ul>
          </div>
    ...
    <>
      ...
      <aside
        className={`hidden lg:flex flex-col fixed left-0 top-0 h-screen
                    bg-slate-900 border-r border-slate-800/70 z-30
